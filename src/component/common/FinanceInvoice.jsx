import { mdiCloseCircleOutline, mdiEye } from '@mdi/js'
...
import tdsicon from "../../assets/images/tdsicon.png"
import Table from './Table'
import RejectLeveltwo from '../../modals/RejectLeveltwo'


const FinanceInvoice = ({ loading, financeInvoiceList,handlePendingList }) => {
  const user = JSON.parse(localStorage.getItem('user'))
  const navigate = useNavigate();
  const modalContext = useContext(ModalContext);
  const { handleModalData } = modalContext;
  const handleViewInvoice = (item) => {
    navigate(`/view-invoice/${item.id}`,{ state: item });
  };
  
  const handleFinanceInvoiceCreation = (id) => {
...
{
                    user?.roles[0].RoleName ==="Finance"  && data.Status ===5
                    &&
                    <>
                    <img src={tdsicon} alt="" onClick={()=>handleFinanceInvoiceCreation(data.id)} />
  ...
  return (
    ...
    {loading ? 
      <Spinner />: 
      <DashboardTable columns={FeedsColumns} data={financeInvoiceList} />
    }
  </div>
  )
}
