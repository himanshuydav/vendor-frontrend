...
import { mdiCheckboxMarkedCircleOutline, mdiCloseCircleOutline, mdiEye } from '@mdi/js';
...
const VpTables = ({excutiveLists,loading,handleVendorList}) => {
 ...
 const handleViewVendorDetails = (item) => {
  ...
  const handleAproveModal =(item ) =>{
    const Addpeople = <VpApporve item={item} handleVendorSort={handleVendorList}   />
    handleModalData(Addpeople, "md")
  }

  const handleRejectModal =(item ) =>{
    const Addpeople = <RejectedLevel item={item} handleVendorList={handleVendorList}  />
    handleModalData(Addpeople, "md")
  }
                ...
                />
                    {
                     user?.roles[0].RoleName ==="VP Operations"  && item.Status === 4
                        &&
                          <>
                         <button className="ms-1 aprove-btn" onClick={()=>handleAproveModal(item)} > 
                         ...
                         <button className="ms-1 delete-btn" onClick={()=>handleRejectModal(item)} >
                          <svg width="31" height="30" viewBox="0 0 31 30" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <div>
      {
        loading ? <Spinner />
        :
        <DashboardTable    columns={FeedsColumns} data={excutiveLists} />
      }
    </div>
  );
};
