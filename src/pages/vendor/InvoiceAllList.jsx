...
const InvoiceAllList = () => {
  ...
  const handleRejectModal = (item) => {
    const Reactpeople = (
      <RejectLeveltwo item={item} handleInvoiceList={handleInvoiceList} />
    );
  ...
  const handleInvoiceList = async () => {
    ...
    InvoiceList(result.res.InvoiceList[0].lines ||result.res.InvoiceList);
  };
  ...
  return (
    <div>
      <Helmet>
        <title>Vendor Portal || Bills</title>
      </Helmet>
      <div className="card">
